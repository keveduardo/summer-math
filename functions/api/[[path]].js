/**
 * Beach Math — server-side auth + logging (Cloudflare Pages Function)
 ...
 *   GET  /api/load                 -> returns this user's saved progress from KV
 *   POST /api/save    { progress } -> saves this user's progress to KV (cross-device sync)
 *
 * Environment variables (set in Pages → Settings → Variables and Secrets):
 *   USERS_JSON    e.g. {"beachfc":{"id":"madeleine","name":"Madeleine"},"test":{"id":"test","name":"Test"}}
 *   TOKEN_SECRET  a long random string (used to sign the session cookie)
 *   SHEET_URL     your Apps Script Web App URL ending in /exec  (kept private here)
 *
 * KV binding (Pages -> Settings -> Bindings -> Add -> KV namespace):
 *   Variable name: BEACH_MATH_KV   ->   a KV namespace you create (e.g. "beach-math")
 */
...
export async function onRequest(context) {
  ...
  if (path === "/api/log" && request.method === "POST") {
    ...
    const row = {
      ...
      name: p.name,                 // trusted from the signed token, not the client
